...
class DANS(GeneralRecommender):

    input_type = InputType.PAIRWISE

    def __init__(self, config, dataset):
        super(DANS, self).__init__(config, dataset)
    ...
    def calculate_loss(self, interaction):
        ...
        n_hard_syth = n_hard * torch.sigmoid(n_hard * u_neg_pre.unsqueeze(dim=1))
        a = torch.norm(torch.mul(s_e.unsqueeze(1), u_neg_pre),dim=-1)
        denoise = torch.mean(torch.sum(a,axis=-1))
